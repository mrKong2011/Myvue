<template>
    <div class="content">
        ...
        <div class="footbox">
            <input type="button" value="登 录" class="login-btn" @click="postUserMs"/>
...
<script>
    export default{
        ...
        methods:{
            postUserMs(){
                let _this = this;
                if(_this.userName.length>5&&_this.userName.passWord!=''){
                    this.$http.post('/use',{userName:_this.userName,passWord:_this.passWord})
                        .then((res)=>{})
                        .catch((err)=>{
                      alert('message: ' + err)
